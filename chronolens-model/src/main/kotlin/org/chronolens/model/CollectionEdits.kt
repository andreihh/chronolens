...
public sealed class SetEdit<T> {
  ...
  public companion object {
     ...
     *
     * @throws IllegalStateException if this set has an invalid state and the given [edits] couldn't
     * be applied
     */
    @JvmStatic
    ...
    /** Utility method. */
    @JvmStatic public fun <T> Set<T>.apply(vararg edits: SetEdit<T>): Set<T> = apply(edits.asList())
  }
  ...
  public data class Add<T>(val value: T) : SetEdit<T>() {
    override fun applyOn(subject: MutableSet<T>) {
      check(subject.add(value)) { "Can't add '$value' because it already exists in '$subject'!" }
    }
  }
